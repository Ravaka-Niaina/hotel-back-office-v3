...
const TABLE_HEAD = [
  { id: 'cancelingPoliticName', label: 'nom politique d\'annulation', alignleft: true },
...
const HistoricPolitic = () => {
  ...
  */
  ...
  const [historicCancelingPoliticList, setHistoricCancelingPoliticList] = useState(new Array(0));
  ...
  const handleChangePage = (e, p, row = rowsPerPage) => {
    console.log(`page = ${  p}`);
    setPage(p + 1);
    fetchHistoricModifCancelingPolitic({nbContent: row, numPage: p + 1});
  }
  const handleChangeRowsPerPage = (e) => {
    const row = parseInt(e.target.value, 10);
    handleChangePage(null, 0, row);
    setRowsPerPage(row);
    fetchHistoricModifCancelingPolitic({nbContent: row, numPage: 1});
  };
  ...
  const fetchHistoricModifCancelingPolitic = ({ nbContent = 5, numPage = 1 }) => {
    ...
    getHistoricModifCancelingPolitic(payload)
      .then(result => {
        console.log(result);
        setHistoricCancelingPoliticList(result.data.list);
  ...
  useEffect(() => {
    // setHistoricCancelingPoliticList(mockupHistoricRoomTypeList);
    fetchHistoricModifCancelingPolitic({});
  }, []);
            ...
            location === 'list' && (
              <>
                ...
                <CustomizedPaperOutside sx={{ ...lightBackgroundToTop, background: '#E3EDF7', p: 5, minHeight: '100vh' }}>
                  ...
                  /> */}
                  {historicCancelingPoliticList && <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                      <Table>
                        <UserListHead
                          ...
                          rowCount={historicCancelingPoliticList.length}
                          numSelected={selected.length}
                        />
                        <TableBody>
                          {
                          ...
                          {historicCancelingPoliticList.map((row) => {
                            const { 
                              _id,
                              cancelingPoliticName, 
                              modifier, 
                              modificationDate, 
                              modifiedField, 
                              oldValue, 
                        ...
                        {
                          !loading && historicCancelingPoliticList.length < 1 && (
                            <TableRow>
...
export default HistoricPolitic;
