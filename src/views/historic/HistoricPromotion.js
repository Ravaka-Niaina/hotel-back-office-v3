...
const HistoricPromotion = () => {
  ...
  */
  ...
  const [historicPromotionList, setHistoricPromotionList] = useState(new Array(0));
  ...
  const handleChangePage = (e, p, row = rowsPerPage) => {
    console.log(`page = ${  p}`);
    setPage(p + 1);
    fetchHistoricModifPromotion({nbContent: row, numPage: p + 1});
  }
  const handleChangeRowsPerPage = (e) => {
    const row = parseInt(e.target.value, 10);
    handleChangePage(null, 0, row);
    setRowsPerPage(row);
    fetchHistoricModifPromotion({nbContent: row, numPage: 1});
  };
  ...
  const fetchHistoricModifPromotion = ({ nbContent = 5, numPage = 1 }) => {
    const payload = {
      nbContent,
      numPage,
    };
    ...
    getHistoricModifPromotion(payload)
      .then(result => {
        console.log(result);
        setHistoricPromotionList(result.data.list);
  ...
  useEffect(() => {
    // setHistoricPromotionList(mockupHistoricRoomTypeList);
    fetchHistoricModifPromotion({});
  }, []);
            ...
            location === 'list' && (
              <>
                ...
                <CustomizedPaperOutside sx={{ ...lightBackgroundToTop, background: '#E3EDF7', p: 5, minHeight: '100vh' }}>
                  ...
                  /> */}
                  {historicPromotionList && <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                      <Table>
                        <UserListHead
                          ...
                          rowCount={historicPromotionList.length}
                          numSelected={selected.length}
                        />
                        <TableBody>
                          {
                          ...
                          {historicPromotionList.map((row) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <TableCellStyled align="left">{promotionName}</TableCellStyled>
                        ...
                        {
                          !loading && historicPromotionList.length < 1 && (
                            <TableRow>
