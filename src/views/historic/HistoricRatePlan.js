...
import { getHistoricModifRatePlan } from '../../services/HistoricModifRatePlan';
...
const TABLE_HEAD = [
  { id: 'ratePlanName', label: 'nom plan tarifaire', alignleft: true },
...
const HistoricRatePlan = () => {
  ...
  */
  ...
  const handleChangePage = (e, p, row = rowsPerPage) => {
    console.log(`page = ${  p}`);
    setPage(p + 1);
    fetchHistoricModifRatePlan({nbContent: row, numPage: p + 1});
  }
  const handleChangeRowsPerPage = (e) => {
    const row = parseInt(e.target.value, 10);
    handleChangePage(null, 0, row);
    setRowsPerPage(row);
    fetchHistoricModifRatePlan({nbContent: row, numPage: 1});
  };
  ...
  const fetchHistoricModifRatePlan = ({ nbContent = 5, numPage = 1 }) => {
    ...
    getHistoricModifRatePlan(payload)
      .then(result => {
        console.log(result);
            ...
            location === 'list' && (
              <>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
                  <CustomizedTitle text="Historique modifications plans tarifaires" size={20} />
                </Stack>
